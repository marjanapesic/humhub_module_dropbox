...
var options = {
	...
	success : function(files) {
    	...
    	if (showWarning != 0) {
    		...
    		'<br/><br/> <input type="checkbox" onclick="changeWarningOnPosting(this)" style="margin-right:5px;"/>' + unset_warning_on_posting_checkbox_label +
    		'</div> <hr/>' +
    		'<div class="modal-footer">' +
    		'<button type="button" class="btn btn-default pull-right" data-dismiss="modal" style="margin-left:10px;">'+ warning_on_posting_modal_cancel + '</button>' +
    		'<button type="button" class="btn btn-info pull-right" data-dismiss="modal" onclick="javascript:addNewFiles();">'+ warning_on_posting_modal_confirm	+ '</button>' +
    		'</div>' +
    		'</div>' +
    		'</div>' + 
    		'</div>';
...
function addToFileList(id, name) {
	...
	$('#' + listId).before('<li class="dropboxInput">'+ name +'<span id="' + id + '" class="dropbox_remove_file" > <i class="fa fa-remove dropbox_file_remove_link"></i></span></li>');

    $(".dropbox_remove_file").off("click");
    $(".dropbox_remove_file").on("click", function () {
    	$(this).parent().remove();
    	$('#' +  listId).val($('#' +  listId).val().replace(","+$(this).attr('id')+",", ','));
    	if($('#' +  listId).val().match("^"+$(this).attr('id')+",")){$('#' +  listId).val($('#' +  listId).val().replace($(this).attr('id')+",", ''));}
        $.ajax({
            type: 'POST',
